...
void *procesar_cliente(void *arg) {
    ...
    if ((n = read(data->fd, buffer, 128)) == -1) {
        ...
        exit(1); // Si un cliente tiene un problema, mata todo por lo que se
                 // utiliza pthread_exit(NULL); para evitarlo
    }
    buffer[n] = 0;
    ...
    data->free = true; // Esto deberia tener mutex alrededor porque tenemos
                       // lectura en getFreeIndex
    return NULL;
}
